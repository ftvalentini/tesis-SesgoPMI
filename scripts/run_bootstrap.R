...
cat(datetime_now(), "To long (this step requires context groups of equal length!)\n")
df_long = df %>%
    select(word, experiment, corpus, all_of(similarity_cols)) %>% 
    # filter(experiment == "mturk-race") %>%
    # slice(1:4) %>%
    unnest(cols = all_of(similarity_cols)) %>%
    pivot_longer(all_of(similarity_cols), names_to = "tmp") %>%
    mutate(
        we = tmp %>% str_extract("(glovewc|sgns|ft)$"),
...
# df_out[, similarity_cols] = df_out %>%
#   select(all_of(similarity_cols)) %>%
#   mutate_all(function(x) map(x, jsonlite::toJSON))

cat(datetime_now(), "Writing output data frame\n")
fwrite(df_out, "results/figures_data.csv")

cat(datetime_now(), "DONE\n")
