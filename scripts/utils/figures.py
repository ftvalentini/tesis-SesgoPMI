...
def join_bias_dfs(df_bias_pmi, df_bias_glovewc, df_bias_sgns, df_bias_ft):
    ...
    # add suffixes to 'wefat_score', 'sims_a','sims_b' fasttext columns
    df_bias_ft = df_bias_ft.rename(
        columns={"wefat_score": "wefat_score_ft", "sims_a": "sims_a_ft", "sims_b": "sims_b_ft"})
    df_bias = pd.merge(
        df_bias, df_bias_ft, how="inner", on=["corpus", "bias", "word", "idx", "word", "freq"])
    return df_bias
...
def add_pvalue(df, **kwargs):
    """Add WEFAT permutation pvalues to bias DFrame
    """
    n_resamples_permut = kwargs.get('n_resamples_permut', np.inf)
    we_names = ["sgns", "ft", "glovewc"]
    for n in we_names:
...
def scatter_plt(
    ...
    edgecolor='black', ylim=(-0.05, 1.05)
):
