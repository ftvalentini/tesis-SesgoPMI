...
SAVE_FILE=$OUT_DIR/cooc-$CORPUS_NAME-$COOC_PARAMS.npz
...
if [[ ! -f $SAVE_FILE ]]; then
  ...
  python3 -u scripts/bin2sparse.py $VOCAB_FILE $COOC_FILE $SAVE_FILE
else
  echo "Sparse matrix: $SAVE_FILE exists. Skipping."
fi
